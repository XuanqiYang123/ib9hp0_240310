...
category_lookup <- 
  data.frame("category_id" = seq(1, length(unique(categories_data$category)),1),
             "category" = unique(categories_data$category))

categories_data <- 
  #lookup category_id
  merge(category_lookup, categories_data, by = "category") %>%
  #reorder the columns
  select(2,3,1,4) %>%
...
memberships_type <- data.frame("membership_type_id" = c(rep(seq(1,3,1),2)),
                               "membership_type" =  c("Student", "Trial", "Premium"))

#Start with the cust_id as the foreign key
memberships_data <- data.frame(customers_data$cust_id)

#Randomly assign membership type to all customers
memberships_data$membership_type <- 
  sample(memberships_type$membership_type, 
         as.numeric(nrow(memberships_data)), replace = T)

#Lookup membership_id
memberships_data <- 
  merge(memberships_type, memberships_data, by = "membership_type") %>%
