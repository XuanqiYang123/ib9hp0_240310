...
#Adding customer birthyear
customers_data$cust_birth_year <- 
...
#Assign product ID, and adding product names
products_data <- 
  conjurer::buildProd(20, minPrice = 1, maxPrice = 100) %>% 
  mutate("prod_name" = sample(gemini_prods$prod_name, 20)) %>%
  left_join(select(gemini_prods, -category), 
            by = join_by(prod_name)) %>%
  rename(prod_id = SKU) %>%
  mutate("prod_id" = gsub("sku", "prod", products_data$prod_id))
```
...
orders_data <- buildPareto(products_data$SKU, orders_data$transactionID, pareto = c(80,20))
```
